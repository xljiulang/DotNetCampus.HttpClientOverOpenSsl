    ...
    static class SocksHelper
    {
        ...
        private static async ValueTask EstablishSocks5TunnelAsync(Stream stream, string host, int port, NetworkCredential? credentials, bool async)
        {
            ...
            try
            {
                    ...
                    case METHOD_USERNAME_PASSWORD:
                        {
                            ...
                            if (credentials is null)
                            {
                                ...
                                throw new IOException("要求必须填写身份信息");
                            }
                            ...
                            {
                                throw new IOException("身份认证失败");
                            }
                            break;
                        }

                    default:
                        throw new IOException("不支持当前使用的认证方式");
                }
                ...
                {
                    throw new IOException($"连接失败，REP：{buffer[1]}");
                }
                int bytesToSkip = buffer[3] switch
                {
                    ...
                    _ => throw new IOException("无效的地址类型")
                };
        ...
        private static async ValueTask EstablishSocks4TunnelAsync(Stream stream, bool isVersion4a, string host, int port, NetworkCredential? credentials, bool async, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                else if (!isVersion4a)
                {
                    ...
                    {
                        throw new IOException("无法解析域名", ex);
                    }

                    if (addresses.Length == 0)
                    {
                        throw new IOException("域名解析结果不包含IPV4地址");
                    }
        ...
        private static byte EncodeString(ReadOnlySpan<char> chars, Span<byte> buffer, string parameterName)
        {
            ...
            catch
            {
                ...
                throw new IOException($"参数{parameterName}的长度过长");
            }
        }
        ...
        private static void VerifyProtocolVersion(byte expected, byte version)
        {
            if (expected != version)
            {
                throw new IOException($"期望的版本为{expected}，实际为{version}");
            }
        }
        ...
        private static async ValueTask ReadToFillAsync(Stream stream, Memory<byte> buffer, bool async)
        {
            while (buffer.Length != 0)
            {
                ...
                if (bytesRead == 0)
                {
                    throw new IOException("远程端响应了0字节内容");
                }
